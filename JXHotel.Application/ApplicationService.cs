...
using JXHotel.Application.Mapper;
using AutoMapper;
...
namespace JXHotel.Application
{
    public abstract class ApplicationService : DisposableObject
    {
            ...
            where TAggregateRoot : class, IAggregateRoot
        {
            ...
            if (dataTransferObjects.Count > 0)
            {
                ...
                foreach (var dto in dataTransferObjects)
                {
                    if (processDto != null)
                        processDto(dto);
                    var ar = AutoMapper.Mapper.Map<TDataObject, TAggregateRoot>(dto);
                ...
                ars.ForEach(ar => result.Add(AutoMapper.Mapper.Map<TAggregateRoot, TDataObject>(ar)));
            }
            return result;
        }
            ...
            where TAggregateRoot : class, IAggregateRoot
        {
            ...
            if (dataTransferObjects.Count > 0)
            {
                ...
                foreach (var dto in dataTransferObjects)
                {
                    ...
                    result.Add(AutoMapper.Mapper.Map<TAggregateRoot, TDataObject>(ar));
                }
                repository.Context.Commit();
            }
            return result;
        ...
        protected void PerformDeleteObjects<TAggregateRoot>(List<string> ids, IRepository<TAggregateRoot> repository, Action<Guid> preDelete = null, Action<Guid> postDelete = null)
            where TAggregateRoot : class, IAggregateRoot
        {
            if (ids == null)
        ...
        public static void Initialize()
        {
            ...
            {
                x.AddProfile<DoMapMProfile>();
                x.AddProfile<MMapDoProfile>();
            });             
